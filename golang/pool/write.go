...
func processAnswer(channel chan string, i int, group int, db *sql.DB) error {
	info := fmt.Sprintf("group:%d mock_answer_%d\n", group, i)
	answer, ok := <-channel
	if !ok {
		logger.Printf("[%s] %s %s\n", "info", info, "answer_chan 通道已关闭")
		return errors.New("answer_chan 通道已关闭")
	}

	var myAnswer Answer
	err := json.Unmarshal([]byte(answer), &myAnswer)
	if err != nil {
		logger.Printf("[%s] %s json数据解析失败,origin:%s %v\n", "error", info, answer, err)
		return err
	}

	err = insertAnswer(myAnswer, db)
	if err != nil {
		logger.Printf("[%s] %s 写入数据库失败,origin:%v %v\n", "error", info, myAnswer, err)
		return err
	}
...
func processStat(channel chan string, i int, group int, db *sql.DB) error {
	...
	if err != nil {
		logger.Printf("[%s] %s %v 转换成int64失败,%v\n", "error", info, examID, err)
		return err
	}

	err = updateStat(paperID, db)
	if err != nil {
		logger.Printf("[%s] %s %v 更新统计信息失败,%v\n", "error", info, paperID, err)
		fmt.Println(err)
		return err
	}
...
// insertAnswer 写入答案记录
func insertAnswer(myAnswer Answer, db *sql.DB) error {
...
func updateStat(paperID int64, db *sql.DB) error {
...
func init() {
	logName := fmt.Sprintf("%s-%s.log", "mock_answer_write", time.Now().Format("2006-01-02"))
...
func main() {
	...
	var answerGoCount int = 8 //处理答案协程数量
	...
	if err != nil {
		logger.Printf("[%s] dsn:%s open mysql error,%v\n", "error", mysqlDsn, err)
		return
	}
	...
	for {
		...
		if err != nil {
			logger.Printf("[%s] redis 服务出问题了\n", "info")
			break
		}

		if num := len(papers); num < 1 {
			logger.Printf("[%s] 没有模考交卷队列数据要处理\n", "info")
			continue
		}

		data = make([]string, 0)
		statArr = make([]string, 0)
		answerChan = make(chan string, 8)
		...
		logger.Printf("[%s] data:%v len:%d\n", "info", data, len(data))
		fmt.Println(current, data, len(data))
		if dataNum := len(data); dataNum >= 1 {
			go func(data []string, channel chan string) {
				for _, m := range data {
					channel <- m
				}
				close(channel)
			...
			go func(n int, channel chan string, num int) {
				...
				for {
					...
					for i := 1; i <= n; i++ {
						go processAnswer(channel, i, s, db)
					}
					s++
				}
			}(answerGoCount, answerChan, dataNum)
		}
